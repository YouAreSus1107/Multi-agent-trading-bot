...
*   **Hybrid Execution Layer**: The **Trader Agent** acts as the crucial execution gate. It requires both macro trend alignment (via moving averages/hybrid technical score) AND microstructure confirmation (VWAP extreme/Volume delta trigger) before allowing a trade to pass. 
*   **Dynamic Trailing Stops**: Automatically sets and ratchets dynamic stop-losses upward driven by the 14-period True Range on 5-minute charts. Stops never retreat.
